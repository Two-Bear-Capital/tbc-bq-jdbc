...
1. **Download Driver JAR** — use the `with-logging` variant, which bundles every
   ...
   ```bash
   wget https://github.com/Two-Bear-Capital/tbc-bq-jdbc/releases/latest/download/tbc-bq-jdbc-4.4.2-with-logging.jar
   ```
...
wget https://github.com/Two-Bear-Capital/tbc-bq-jdbc/releases/latest/download/tbc-bq-jdbc-4.4.2-shaded.jar
```
...
```gradle
dependencies {
    implementation 'vc.tbc:tbc-bq-jdbc:4.4.2'
}
```
...
|----------|------|----------|
| `target/tbc-bq-jdbc-4.4.2.jar` | ~220 KB | Driver classes only; dependencies must be on the classpath |
| `target/tbc-bq-jdbc-4.4.2-shaded.jar` | ~41 MB | All dependencies relocated; bring your own SLF4J binding |
| `target/tbc-bq-jdbc-4.4.2-with-logging.jar` | ~42 MB | Shaded, plus Logback preconfigured — the IDE variant |
| `target/tbc-bq-jdbc-4.4.2-sources.jar` | ~185 KB | Sources |
| `target/tbc-bq-jdbc-4.4.2-javadoc.jar` | ~610 KB | API reference |
