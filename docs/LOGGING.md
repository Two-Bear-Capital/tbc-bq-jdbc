...
### 1. Standard JAR (`tbc-bq-jdbc-1.0.54.jar`)
...
### 2. Shaded JAR (`tbc-bq-jdbc-1.0.54.jar`)
...
### 3. Shaded JAR with Logging (`tbc-bq-jdbc-1.0.54-with-logging.jar`)
...
1. **Add the driver** to IntelliJ's database configuration:
   ...
   - Select the `tbc-bq-jdbc-1.0.54-with-logging.jar` file
