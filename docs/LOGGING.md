...
### 1. Standard JAR (`tbc-bq-jdbc-1.0.8.jar`)
...
### 2. Shaded JAR (`tbc-bq-jdbc-1.0.8.jar`)
...
### 3. Shaded JAR with Logging (`tbc-bq-jdbc-1.0.8-with-logging.jar`)
...
1. **Add the driver** to IntelliJ's database configuration:
   ...
   - Select the `tbc-bq-jdbc-1.0.8-with-logging.jar` file
...
```groovy
dependencies {
    implementation 'com.twobearcapital:tbc-bq-jdbc:1.0.8'
