...
translation, and the networking a restricted corporate estate needs — HTTP proxy, custom TLS
truststore, Private Service Connect. It also implements the optional JDBC pooling API and comes
with a support relationship.

So:

**Choose Google's driver if** you are on Java 8–17, connect through a proxy or a TLS-inspecting
middlebox, want `ConnectionPoolDataSource` rather than an external pool, need `CallableStatement`
or OpenTelemetry today, or want first-party support.
...
| Area | Detail |
|---|---|
| **HTTP proxy** *(tracked — [#266][i266])* | `ProxyHost`, `ProxyPort`, `ProxyUid`, `ProxyPwd`, propagated to the auth library as well as the API client, and exercised by a dockerised proxy integration suite. `tbc-bq-jdbc` has no proxy support at all. |
...
[i266]: https://github.com/Two-Bear-Capital/tbc-bq-jdbc/issues/266
