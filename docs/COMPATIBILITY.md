...
| `RANGE` | ⚠️ | `getString()` returns the BigQuery literal, e.g. `[2020-01-01, 2020-12-31)`, with `UNBOUNDED` for an absent bound. `getObject()` returns the client's `Range` object. Reads through the Storage Read API path like any other type. A RANGE nested inside a STRUCT or ARRAY still throws from `getString()` ([#260](https://github.com/Two-Bear-Capital/tbc-bq-jdbc/issues/260)) |
