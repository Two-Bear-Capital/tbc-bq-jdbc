...
|----------|-----------|------|-------------|
| `tbc-bq-jdbc-1.0.73.jar` | *(none)* | ~60KB | Slim JAR (requires dependencies) |
| `tbc-bq-jdbc-1.0.73.jar` | `shaded` | ~38MB | Fat JAR with all dependencies |
| `tbc-bq-jdbc-1.0.73-with-logging.jar` | `with-logging` | ~39MB | Fat JAR + Logback (for IntelliJ) |
| `tbc-bq-jdbc-1.0.73.jar` | `sources` | ~200KB | Source code |
| `tbc-bq-jdbc-1.0.73.jar` | `javadoc` | ~500KB | Javadoc |
...
```groovy
dependencies {
    implementation 'vc.tbc:tbc-bq-jdbc:1.0.73'

    // Or shaded variant
    implementation 'vc.tbc:tbc-bq-jdbc:1.0.73:shaded'
}
```
...
wget https://repo1.maven.org/maven2/vc/tbc/tbc-bq-jdbc/1.0.73/tbc-bq-jdbc-1.0.73.jar
```
