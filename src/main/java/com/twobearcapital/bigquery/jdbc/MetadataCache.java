...
final class MetadataCache {
   ...
   * @return Optional containing the cached ResultSet if available and not expired, empty otherwise
   */
   ...
   *
   * <p>Note: This method reads the entire ResultSet to cache its contents. The ResultSet cursor
   * will be positioned before the first row when this method returns.
   *
   * @param key the cache key
   ...
   *
   * <p>This method removes all cached metadata results immediately.
   */
  void clear() {
    int size = cache.size();
    cache.clear();
   ...
   *
   * <p>This is useful for invalidating related entries, for example all table-related caches for a
   * specific dataset.
   *
   ...
   *
   * <p>This record stores the actual data rather than a ResultSet object, since ResultSets are
   * stateful and can only be read once.
   */
  private record CacheEntry(
      String[] columnNames, int[] columnTypes, List<Object[]> rows, Instant expiresAt) {
