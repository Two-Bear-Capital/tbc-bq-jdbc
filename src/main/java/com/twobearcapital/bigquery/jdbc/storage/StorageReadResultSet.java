...
/**
 * ResultSet implementation using BigQuery Storage Read API for improved performance.
 *
 * <p>The Storage Read API provides:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>This is automatically used when:
 *
 * <ul>
...
public class StorageReadResultSet extends BQResultSet {
  ...
  public StorageReadResultSet(BQStatement statement, TableId tableId)
    ...
    try {
      ...
      logger.info(
          "Created Storage API read session: {} with {} streams",
          readSession.getName(),
  ...
  private ReadSession createReadSession() {
    ...
    CreateReadSessionRequest request =
        CreateReadSessionRequest.newBuilder()
            ...
            .setMaxStreamCount(1) // Start with 1 stream, can be enhanced for parallel
            .build();
   ...
   *
   * @param tableResult the query result
   * @param useStorageApiSetting the useStorageApi connection property
  ...
  public static boolean shouldUseStorageApi(TableResult tableResult, String useStorageApiSetting) {
    ...
    if ("auto".equalsIgnoreCase(useStorageApiSetting)) {
      ...
      if (shouldUse) {
        logger.info(
            "Auto-enabling Storage API for large result: {} rows (~{} MB)",
            totalRows,
