...
/**
 * Abstract base class for BigQuery statement implementations.
 * Provides common query execution logic with template methods for customization.
 * Eliminates ~60 lines of duplicate code between BQStatement and BQPreparedStatement.
 *
 * <p>Fixes:
 * <ul>
...
public abstract class AbstractBQStatement extends BaseCloseable implements Statement {
    ...
    /**
     * Builds the query configuration for execution.
     * Template method for subclasses to customize query config.
     *
     * @param sql the SQL query (may be template with ? placeholders)
     * @return the query configuration builder
     */
    protected abstract QueryJobConfiguration.Builder buildQueryConfig(String sql);

    /**
     * Creates a ResultSet from the query result.
     * Template method for subclasses to customize result set creation.
     *
     * @param result the table result from BigQuery
    ...
    /**
     * Returns the log message prefix for this statement type.
     * Used to differentiate log messages between Statement and PreparedStatement.
     *
    ...
    /**
     * Common query execution logic with resource leak fix.
     * Closes previous ResultSet before creating new one.
     * Thread-safe access to currentJob for cancel operations.
     *
