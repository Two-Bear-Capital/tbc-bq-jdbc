 ...
 *
 * <p>Measures the performance of iterating through result sets of various sizes.
 *
 * @since 1.0.0
 */
...
public class ResultSetIterationBenchmark {
  ...
  public void benchmarkResultSetIteration(Blackhole blackhole) throws Exception {
    String query =
        String.format(
            "SELECT row_num, "
                + "CONCAT('name_', CAST(row_num AS STRING)) as name, "
                + "row_num * 100 as value, "
                + "MOD(row_num, 2) = 0 as is_even "
                + "FROM UNNEST(GENERATE_ARRAY(1, %d)) as row_num",
            rowCount);
