 ...
 *
 * <p>This class sets up a BigQuery emulator container for integration testing. Tests can be run
 * against the emulator or against a real BigQuery instance if credentials are provided.
 *
 * @since 1.0.0
 */
@Testcontainers
public abstract class AbstractBigQueryIntegrationTest {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(AbstractBigQueryIntegrationTest.class);
  ...
  protected static final GenericContainer<?> bigqueryEmulator =
      new GenericContainer<>(DockerImageName.parse("ghcr.io/goccy/bigquery-emulator:latest"))
  ...
  void setup() throws SQLException {
    ...
    String jdbcUrl =
        String.format(
            "jdbc:bigquery://%s:%d;ProjectId=%s;DefaultDataset=%s;OAuthType=0",
   ...
   *
   * <p>Override this method to use real BigQuery credentials in production tests.
   *
  ...
  protected void insertTestData(String tableName) throws SQLException {
    String insert =
        String.format(
            "INSERT INTO %s (id, name, age, salary, is_active, created_date) VALUES "
                + "(1, 'Alice', 30, 75000.50, true, DATE '2024-01-15'), "
                + "(2, 'Bob', 25, 60000.00, true, DATE '2024-02-20'), "
                + "(3, 'Charlie', 35, 85000.75, false, DATE '2024-03-10')",
            tableName);
