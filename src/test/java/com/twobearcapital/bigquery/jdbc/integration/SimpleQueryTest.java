...
class SimpleQueryTest extends AbstractBigQueryIntegrationTest {
  ...
  void testSelectWithAggregate() throws SQLException {
    ...
    String sql =
        "SELECT COUNT(*) as count, AVG(age) as avg_age, MAX(salary) as max_salary FROM users";
    try (Statement stmt = connection.createStatement();
        ResultSet rs = stmt.executeQuery(sql)) {
  ...
  void testSelectWithGroupBy() throws SQLException {
    ...
    // When: Using GROUP BY
    String sql =
        "SELECT is_active, COUNT(*) as count FROM users GROUP BY is_active ORDER BY is_active";
    try (Statement stmt = connection.createStatement();
        ResultSet rs = stmt.executeQuery(sql)) {
